...
def generate_result_report(output_dir, benchmark_result_list,
                           n_gpus="na", scale="na", edgefactor="na",
                           orc_dir="na"):
    """
    Create a <algo>_benchmark_results.txt file for each result in the
    output_dir.
    """
    for r in benchmark_result_list:
        name = r.name
        report_path = Path(output_dir) / f"{name}_benchmark_results.txt"
        with open(report_path, "w") as f:
            f.write("algo, seconds, n_gpus, scale, edgefactor, orc_dir\n")
            f.write(f"{name}, {r.runtime:.6}, {n_gpus}, {scale}, "
                    f"{edgefactor}, {orc_dir}\n")
